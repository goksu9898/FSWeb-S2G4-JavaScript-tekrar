...
/* 	GÖREV 3:
		- Sayfanın en üstünde global değişken olarak tanımlanmış bir sayilar dizisi bulunmaktadır. Bu dizi içinde 0 ile 1000 arasında rasgele oluşturulmuş 
			tam sayılar ve ondalıklı sayılar bulunmaktadır. Bu diziyi kullanarak aşağıdakileri uygulayın:
...
ucebolunenlerintoplami = ucetambolunenler.reduce(getSum, 0);
...
siralisayilar = besyuzdenkucuksayilar.sort(compareNumbers);
...
for (let i = 0; i < sayilar.length; i++) {
	...
	let resultElement = nesneList.find((element) => element.sayi == sayilar[i]);
	
	// function metot (element, sayilar[i]) {
	// 	if (element.sayi == sayilar[i]) {
	// 		return true;
	// 	}
	// 	return false;
	// }
	...
	if(resultElement != undefined) {
		...
		nesneList.push(nesne);
	}
}

for (let i = 0; i < nesneList.length; i++) {
	tekraredensayilar.push(nesneList[i].sayi + " sayısı " + nesneList[i].tekrar + " kere tekrar edilmiştir");
}
